...
namespace Jint.Runtime
{
 ...
 public class TypeConverter
 {
	...
	public static IEnumerable<MethodData> FindBestMatch(Engine engine, List<MethodData> methods, JsValue[] arguments)
	{
	 methods = methods
			 .Where((m) =>
			 {
				if (m.ParameterTypes.Length == arguments.Length)
				 return true;

				if (m.ParamsParameterIndex.HasValue && (m.ParameterTypes.Length < arguments.Length || m.ParamsParameterIndex.Value <= arguments.Length))
				 return true;

				return false;
			 }).ToList();

	 if (methods.Count == 1 && !methods[0].ParameterTypes.Any())
	 {
